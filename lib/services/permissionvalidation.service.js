...
const { TOKEN_MISSING, PERMISSION_MISSING, SOURCE_INVALID, SOURCE_MISSING, PERMISSION_FORMAT_INVALID } = require('../errors/error.messages');
...
async function checkPermission(authToken, requiredPermissions, requestedsource) {
  ...
  const permissions = await permissionsService.getPermissions(token, source);
